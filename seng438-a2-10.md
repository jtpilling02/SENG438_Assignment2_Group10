...
# Introduction <a name="introduction"></a>
  ...
  In this lab session our team managed to develop our own test cases using black box strategies, then used JUnit to perform the test cases themselves.

Prior to this lab session, we had very little practical experience with creating our own test cases, and we also had little JUnit experience and more specifically mocking a test case. We overall understood the concepts involved with black box testing such as equivalence class and boundary value testing which helped us develop our test cases. 
...
The final strategy, written for ***Partition Four*** used equivalence class and boundary value testing to generate test cases. This was split into positive numbers and negative numbers, the minimum and maximum values for an int, and also null values.
...
|onecolumn_calculateRowTotal()|Mock 2D Array with 1 Column| Som of row 1|
...
When looking back on mocking during a test case, it has many advantages and disadvantages when being used. Some advantages of mocking is that they are both fast and stable across systems. Without needing to use outside services the test will always be run the same way every time. Isolation is also a large benefit of mocking, as you can isolate if the reasoning behind a failure is a dependency. The biggest issue with a mock is that they are not actually equivalent to the integration that they are mocking. Because of this, you are cannot be certain that the test will work. They also may create false-negatives if set up inaccurately.
